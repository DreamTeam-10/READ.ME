...
Full detail on each framework's scope and canonical docs: [`docs/framework-reference.md`](docs/framework-reference.md).

## Repo structure

```
READ.ME/
├── docs/               → framework reference & analytical methodology
├── governance/         → AI Agent Governance RACI matrix + narrative guide
├── case-studies/       → incident/policy breakdowns, mapped to frameworks
└── resources/          → tools, standards bodies, and training links used across this work
```

## How the analysis works

Every case study follows the same structure: a real incident or policy development is the entry point, the framework stack determines the lens (not the other way around), and each writeup closes with concrete governance questions rather than a forward-looking teaser. Details in [`docs/methodology.md`](docs/methodology.md).
